...
| **Logical** | **Business Logic Failure in Moderation.** <br> The moderation "DELETE" action was merely flagging `IsHidden = true` (Soft Delete) instead of physically removing the database row (Hard Delete). <br><br> *See image: [Logical Error 3](./images/bug5-broken-delete.png)* | After testing in Postman noticed messages hiding instead of deleting. Looked through the method, and replaced the incorrect soft-delete assignment with the correct Entity Framework command: `_context.Messages.Remove(message);`. |
